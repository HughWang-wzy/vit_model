...
def main(args):
    ...
    # 加载学生模型权重
    best_model_path = "vit_models_v1.4_dynamic_vit/train1/best_model_dynamic_vit_kd.pth"
    if os.path.exists(best_model_path):
        print(f"从 {best_model_path} 加载学生模型权重...")
        student_model.load_state_dict(torch.load(best_model_path, map_location=device))
        print("学生模型权重加载完成。")
    else:
        print(f"未找到权重文件: {best_model_path}，将从头开始训练。")
