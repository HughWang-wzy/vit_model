...
def main(args):
    ...
    #     depths=[8, 4, 4, 2],  # Swin-T 深度
    #     num_heads=[3, 6, 12, 24], # Swin-T 头数
    #     window_size=4,      # 关键：窗口大小改为 4 (因为 8x8 特征图)
    #     drop_path_rate=0.00, # DropPath 比 Dropout 更常用
    #     # ape=False # 通常 Swin 不用 APE
    # ).to(device)
    # print(f"模型参数: {sum(p.numel() for p in model.parameters() if p.requires_grad)/1e6:.2f}M")

    model = CoAtNet(
        img_size=32,
        num_classes=10,
        block_types=['C', 'C', 'T', 'T'], # 2 Conv stages, 2 Transformer stages
        dims=[64, 128, 256, 512],     # Stage output dimensions
        depths=[2, 2, 4, 2],         # Blocks per stage (slightly adjusted)
        transformer_heads=8,         # Heads for Transformer stages
        transformer_dim_head=32,     # Dim per head (256/8=32, 512/8=64 - adjust if needed)
        mbconv_expand_ratios=[4, 4], # Expand ratio for Conv stages
        mbconv_kernel_sizes=[3, 3],  # Kernel size for Conv stages
        drop_path_rate=0.05,          # Stochastic depth rate
        dropout=0.05                  # Dropout in Transformer MLP
    ).to(device)

    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"使用的模型: CoAtNet (CIFAR Config)")
    ...
    train_dir = get_next_train_dir(base_dir="./coatnet_models_v0.1")
    ...
    pretrained_path = "coatnet_models_v0.1/train1/best_model_vit_stage1.pth"
