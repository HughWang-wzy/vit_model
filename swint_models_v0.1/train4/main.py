...
def main(args):
    ...
    model = SwinTransformer(
        ...
        depths=[2, 2, 6, 2],  # Swin-T 深度
        num_heads=[3, 6, 12, 24], # Swin-T 头数
        window_size=4,      # 关键：窗口大小改为 4 (因为 8x8 特征图)
        drop_path_rate=0.1, # DropPath 比 Dropout 更常用
    ...
    mixup_cutmix = RandomChoice(
        transforms=[
            Mixup(num_classes=args.num_classes, mixup_alpha=0.3, cutmix_alpha=0.0),
            Mixup(num_classes=args.num_classes, mixup_alpha=0.0, cutmix_alpha=0.4)
        ],
    ...
    train_dir = get_next_train_dir(base_dir="./swint_models_v0.1")
    ...
    pretrained_path = "swint_models_v0.1/train3/best_model_vit_stage1.pth"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, default='vanilla_vit',
                        ...
                        help='选择要运行的模型类型')
    parser.add_argument('--batch_size', type=int, default=128*2, help='批处理大小')
